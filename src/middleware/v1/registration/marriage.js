  ...
  read: (req, res, next) => {
    ...
    if (!ri.client || !ri.username) {
      ...
      audit.create(ri.username, ri.client, req.url, ri.groups, 'lookup', 'marriage')
        ...
        .then(r => {
          if (r) {
            res.send(censorMarriage(r, ri.roles));
            metrics.lookup('marriage', ri.username, ri.client, ri.groups, ri.roles, startTime, moment(), id, r.status.blocked);
            next();
          } else {
    ...
    if (!ri.client || !ri.username) {
      ...
      if (dom && !dom.isValid()) {
        ...
        audit.create(ri.username, ri.client, req.url, ri.groups, 'search', 'marriage')
          ...
          .then(r => {
            res.send(r.map(e => censorMarriage(e, ri.roles)));
